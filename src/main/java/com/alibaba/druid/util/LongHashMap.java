...
public class LongHashMap<V> implements Map<Long, V>, Cloneable, Serializable {
    ...
    public boolean containsKey(Object key) {
        return containsKey(((Long) key).intValue());
    }
    ...
    final Entry<V> getEntry(long key) {
        int hash = hash(key);
        for (Entry<V> e = table[indexFor(hash, table.length)]; e != null; e = e.next) {
            if (e.hash == hash && key == e.key) return e;
        }
        return null;
    }
    ...
    private void putAllForCreate(Map<? extends Long, ? extends V> m) {
        for (Iterator<? extends Map.Entry<? extends Long, ? extends V>> i = m.entrySet().iterator(); i.hasNext();) {
     ...
     * does not resize the map, but sets threshold to Long.MAX_VALUE. This has the effect of preventing future calls.
     * 
    ...
    final Entry<V> removeMapping(Object o) {
        ...
        Map.Entry<Long, V> entry = (Map.Entry<Long, V>) o;
        int key = ((Long) entry.getKey()).intValue();
        int hash = key;
        int i = indexFor(hash, table.length);
        Entry<V> prev = table[i];
        Entry<V> e = prev;
    ...
    static class Entry<V> implements Map.Entry<Long, V> {
    ...
    private final class EntryIterator extends HashIterator<Map.Entry<Long, V>> {
    ...
    Iterator<Map.Entry<Long, V>> newEntryIterator() {
        return new EntryIterator();
    }

    // Views

    private transient Set<Map.Entry<Long, V>> entrySet = null;
    ...
    private final class KeySet extends AbstractSet<Long> {
        ...
        public boolean remove(Object o) {
            return LongHashMap.this.removeEntryForKey(((Long) o).intValue()) != null;
        }
    ...
    private Set<Map.Entry<Long, V>> entrySet0() {
     ...
     * <i>size</i> (an int, the number of key-value mappings), followed by the key (Object) and value (Object) for each
     * key-value mapping. The key-value mappings are emitted in no particular order.
     */
    private void writeObject(java.io.ObjectOutputStream s) throws IOException {
        Iterator<Map.Entry<Long, V>> i = (size > 0) ? entrySet0().iterator() : null;
