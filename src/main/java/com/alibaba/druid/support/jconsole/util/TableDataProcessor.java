...
public final class TableDataProcessor {
	...
	/** The Constant COLUMN_VALUE_NAME. */
	private static final String COLUMN_VALUE_NAME = "值";
	
	/**
	 * Instantiates a new table data processor.
	 */
	...
	/**
	 * 将行数据转化为列数据
	 * 由于json中的数据是按照一条条的记录返回的，而在显示的时候需要按照“名称”、“值”两列显示，所以要做转化。
	 *
	 * @param rowDatas 原始数据
	...
	/**
	 * 将行数据转化为多个表格中的列数据
	 * 和{@link #row2col(ArrayList<LinkedHashMap<String,Object>>, String)}类似，
	 * 只不过这里是返回多个表格数据 
	 *
	 * @param rowDatas 原始数据
	...
	public static class ColumnData {
		...
		/**
		 * Instantiates a new column data.
		 */
		public ColumnData() {
			super();
		}
