...
public class PGSelectTest73_filter extends TestCase {
    ...
    public void test_0() throws Exception {
        ...
        System.out.println(SQLUtils.toPGString(stmt, SQLUtils.DEFAULT_LCASE_FORMAT_OPTION));
        ...
        assertEquals("SELECT a.*, AVG(a.nv_roe_lag) OVER () AS nv_roe_avg, AVG(a.idx_roe_lag) OVER (ORDER BY a.stat_date) AS idx_roe_avg\n" +
                ...
                "\tEND AS nv_flag_up, COUNT(1) FILTER (WHERE a.nv_flag <= 0) OVER (ORDER BY a.stat_date) AS nv_flag_down, COUNT(1) FILTER (WHERE a.idx_flag >= 0) OVER (ORDER BY a.stat_date) AS idx_flag_up\n" +
                "\t, COUNT(1) FILTER (WHERE a.idx_flag <= 0) OVER (ORDER BY a.stat_date) AS idx_flag_down, COUNT(1) FILTER (WHERE a.if300_flag >= 0) OVER (ORDER BY a.stat_date) AS if300_flag_up\n" +
                "\t, COUNT(1) FILTER (WHERE a.if300_flag <= 0) OVER (ORDER BY a.stat_date) AS if300_flag_down, COUNT(1) FILTER (WHERE a.ih50_flag >= 0) OVER (ORDER BY a.stat_date) AS ih50_flag_up\n" +
                "\t, COUNT(1) FILTER (WHERE a.ih50_flag <= 0) OVER (ORDER BY a.stat_date) AS ih50_flag_down, COUNT(1) FILTER (WHERE a.ic500_flag >= 0) OVER (ORDER BY a.stat_date) AS ic500_flag_up\n" +
                "\t, COUNT(1) FILTER (WHERE a.ic500_flag <= 0) OVER (ORDER BY a.stat_date) AS ic500_flag_down, COUNT(1) FILTER (WHERE a.idx_roe_lag > 0) OVER (ORDER BY a.stat_date) AS idx_win_cnt\n" +
                "\t, COUNT(1) FILTER (WHERE a.idx_roe_lag > a.idx_roe_lag) OVER (ORDER BY a.stat_date) AS idx_win_idx_cnt, COUNT(1) FILTER (WHERE a.nv_roe_lag > 0) OVER (ORDER BY a.stat_date) AS nv_win_cnt\n" +
                "\t, COUNT(1) FILTER (WHERE a.nv_roe_lag > a.idx_roe_lag) OVER (ORDER BY a.stat_date) AS nv_win_idx_cnt, COUNT(1) FILTER (WHERE a.if300_roe_lag > 0) OVER (ORDER BY a.stat_date) AS if300_win_cnt\n" +
                "\t, COUNT(1) FILTER (WHERE a.if300_roe_lag > a.idx_roe_lag) OVER (ORDER BY a.stat_date) AS if300_win_idx_cnt, COUNT(1) FILTER (WHERE a.ih50_roe_lag > 0) OVER (ORDER BY a.stat_date) AS ih50_win_cnt\n" +
                "\t, COUNT(1) FILTER (WHERE a.ih50_roe_lag > a.idx_roe_lag) OVER (ORDER BY a.stat_date) AS ih50_win_idx_cnt, COUNT(1) FILTER (WHERE a.ic500_roe_lag > 0) OVER (ORDER BY a.stat_date) AS ic500_win_cnt\n" +
                "\t, COUNT(1) FILTER (WHERE a.ic500_roe_lag > a.idx_roe_lag) OVER (ORDER BY a.stat_date) AS ic500_win_idx_cnt\n" +
                ...
                "FROM nv_a a", SQLUtils.toPGString(stmt));
        ...
        assertEquals("select a.*, avg(a.nv_roe_lag) over () as nv_roe_avg, avg(a.idx_roe_lag) over (order by a.stat_date) as idx_roe_avg\n" +
                ...
                "\t, stddev_samp(a.ih50_roe_lag - a.idx_roe_lag) over (order by a.stat_date) as ih50_stderr\n" +
                "\t, stddev_samp(a.ic500_roe_lag - a.idx_roe_lag) over (order by a.stat_date) as ic500_stderr\n" +
                "from nv_a a", SQLUtils.toPGString(stmt, SQLUtils.DEFAULT_LCASE_FORMAT_OPTION));
