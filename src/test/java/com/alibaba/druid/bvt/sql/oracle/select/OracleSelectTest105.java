    ...
    public void test_0() throws Exception {
        String sql = //
                "SELECT gz, UTL_RAW.CAST_TO_RAW(gzname) AS gzname, dept\n" +
                        "\t, UTL_RAW.CAST_TO_RAW(deptname) AS deptname, floor\n" +
                        "\t, UTL_RAW.CAST_TO_RAW(floorname) AS floorname, area, storeno\n" +
                        "FROM sys_floor_dept_gz";
...
//                instanceof SQLAggregateExpr);
...

        {
            ...
            assertEquals("SELECT gz, UTL_RAW.CAST_TO_RAW(gzname) AS gzname, dept\n" +
                    "\t, UTL_RAW.CAST_TO_RAW(deptname) AS deptname, floor\n" +
                    "\t, UTL_RAW.CAST_TO_RAW(floorname) AS floorname, area, storeno\n" +
                    "FROM sys_floor_dept_gz", text);
        }
