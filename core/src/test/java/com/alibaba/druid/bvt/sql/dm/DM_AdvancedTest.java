...
public class DM_AdvancedTest extends TestCase {
    ...
    public void test_recursive_cte() {
        // Oracle/DM uses CONNECT BY for hierarchical queries instead of RECURSIVE
    ...
    public void test_window_lag_lead() {
        String sql = "SELECT id, value, LAG(value, 1) OVER (ORDER BY id) as prev_val, LEAD(value, 1) OVER (ORDER BY id) as next_val FROM data";
        List<SQLStatement> stmtList = SQLUtils.parseStatements(sql, DbType.dm);
        assertEquals(1, stmtList.size());
    }
