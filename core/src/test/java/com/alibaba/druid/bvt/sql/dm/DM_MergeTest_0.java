...
public class DM_MergeTest_0 extends TestCase {
    public void test_0() throws Exception {
        ...
        assertTrue(stmt instanceof SQLMergeStatement);

        SQLMergeStatement mergeStmt = (SQLMergeStatement) stmt;
        assertNotNull(mergeStmt.getInto());
        assertNotNull(mergeStmt.getUsing());
        assertNotNull(mergeStmt.getOn());
        assertEquals(2, mergeStmt.getWhens().size());
        assertTrue(mergeStmt.getWhens().get(0) instanceof SQLMergeStatement.WhenUpdate);
        assertTrue(mergeStmt.getWhens().get(1) instanceof SQLMergeStatement.WhenInsert);

        OracleSchemaStatVisitor visitor = new OracleSchemaStatVisitor();
        stmt.accept(visitor);
        assertTrue(visitor.getTables().containsKey(new TableStat.Name("sys_user_online")));
