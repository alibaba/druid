...
Parser internal refactoring SHALL preserve externally observable parsing behavior, including Snowflake token-consumption refactors, `SQLStatementParser` giant-method decomposition changes, `SQLASTOutputVisitor` decomposition, and `SQLExprParser.primary()` decomposition. Refactor verification SHALL also include structured, comparable baseline/post-change regression evidence for touched parser paths.
